...
# Based on official ACE-Step v1.5 documentation
# ============================================================================
# Optimal settings for 8GB VRAM:
# - DiT: acestep-v15-turbo (8 steps, ~3GB VRAM)
# - LM: acestep-5Hz-lm-1.7B with vllm backend (~4GB VRAM)
# - FP16: true (50% VRAM saving)
# - Tiled Decode: true (default, prevents OOM)
# - Batch Size: 1
# ============================================================================
...
# Environment variables MUST be set in the same cmd.exe context
Write-Host "[3/5] Starting ACE-Step API (port 8001)..." -ForegroundColor Cyan
$aceStepCmd = @'
cd /d D:\VocalForge\ace-step
title VocalForge ACE-Step API
call .venv\Scripts\activate.bat
REM ACE-Step v1.5 Configuration for RTX 3070 8GB
...
set ACESTEP_FP16=true
set ACESTEP_USE_TILED_DECODE=true
set ACESTEP_BATCH_SIZE=1
set ACESTEP_AUTH_DISABLED=1
python acestep/api_server.py --host 0.0.0.0 --port 8001
'@
Start-Process "cmd.exe" -ArgumentList "/k $aceStepCmd"
...
Write-Host "  - LM Model: acestep-5Hz-lm-1.7B (~4GB)" -ForegroundColor Gray
Write-Host "  - FP16: Enabled (50% VRAM saving)" -ForegroundColor Gray
Write-Host "  - Tiled Decode: Enabled" -ForegroundColor Gray
Write-Host "  - Expected VRAM usage: ~6-7GB / 8GB" -ForegroundColor Gray
Write-Host ""
